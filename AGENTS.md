...
| OpenFreeMap zoom range | The planet TileJSON reports **`maxzoom: 14`** — every zoom above that is overzoomed client-side from the z14 tile. `MAX_ZOOM` (19) is a PINCH limit, not a tile level, and the two are constantly confused: at z14 the whole campus is about **four tiles**, so an offline pack of CLSU's vector data is well under a megabyte. The tile URL inside that TileJSON is also **versioned** (`/planet/<YYYYMMDD_HHMMSS>_pt/{z}/{x}/{y}.pbf`), so inlining it as `tiles: [...]` to avoid the TileJSON round trip pins the app to a release that will be retired — do not |
| MapLibre offline packs | `OfflinePackCreateOptions.mapStyle` is `string` ONLY: Android passes it straight to `OfflineTilePyramidRegionDefinition` (MLRNOfflineModule.kt:424), bypassing the helper that writes a style object to a temp file. So a style object has to be written out and handed over as a `file://` URI — which is also the seam that lets a pack drop resources. ⚠️ **The downloader requests EVERY glyph range of EVERY fontstack the style names** — 256 ranges each, measured at ~93 KB apiece on OpenFreeMap. A style with `glyphs` therefore costs ~40-50 MB of type to fetch ~1 MB of tiles. `map-style.ts`'s `tilePackStyle()` strips `glyphs`, `sprite` and the symbol layers for exactly that reason; labels come from the **ambient cache** instead, which stores everything the map has already drawn. Both `minZoom` and `maxZoom` must be passed — the defaults are 10/20 |
...
src/
  ...
  components/       design system — the ONLY UI surface app code imports
    ...
    common/         app-wide composites that know this app's brand or domain
      map.tsx            the only file naming MapLibre's VIEW APIs
      offline-map.ts     the only other MapLibre file: the tile pack, no UI
